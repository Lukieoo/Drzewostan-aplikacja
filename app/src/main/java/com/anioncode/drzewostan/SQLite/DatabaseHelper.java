...
public class DatabaseHelper extends SQLiteOpenHelper {
    public DatabaseHelper(Context context) {
        super(context, "las.db", null, 1);
    }

    @Override
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        sqLiteDatabase.execSQL("CREATE TABLE PINE(ID INTEGER PRIMARY KEY AUTOINCREMENT,SREDNICA TEXT UNIQUE,KLASA_1 INTEGER,KLASA_2 INTEGER,KLASA_3 INTEGER,KLASA_A " +
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {
        ...
        sqLiteDatabase.execSQL("DROP TABLE IF EXISTS PINE");
    ...
    public boolean insertData(String srednica,int klasa_1,int klasa_2,int klasa_3,int klasa_a,int klasa_b,int klasa_c,int wysokosc){
        ...
        long result=db.insert("PINE",null,contentValues);
    ...
    public Cursor LasDane(int id){
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "SELECT * FROM PINE WHERE ID="+id;
        Cursor data = db.rawQuery(query, null);
        return data;
    }
    public Cursor AllData(){
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "SELECT * FROM PINE ";
        Cursor data = db.rawQuery(query, null);
        return data;
    }
    public ArrayList<Trees> getAllData(){
        ...
        Cursor cursor =db.rawQuery("Select * from PINE",null);
    ...
    public boolean updateData(String klasa,int id,int liczba) {
        ...
        db.update("PINE", contentValues, "ID = ?",new String[] { String.valueOf(id) });
        return true;
    }
    ...
    public void deleteName(){
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "DELETE FROM PINE WHERE id>=0";
        db.execSQL(query);
    }
}
